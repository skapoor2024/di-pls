...
def w_pls_nipals_di(x,y,D,lA,I,iter):
    ...
    w_pls_old = 100
    for i in range(iter):
        w_pls = np.dot(x.T,y_score)/np.dot(y_score,y_score)
        #w_pls = w_pls/np.linalg.norm(w_pls)
        #w_pls /= np.sqrt(np.dot(w_pls,w_pls)+eps)
        #gamma = (np.linalg.norm((x-y@w_pls))**2)/(w_pls@D@w_pls.T)
        w_pls = np.expand_dims(w_pls,0)
        #g = x - np.dot(y,w_pls)
        #g_2 = np.sqrt(np.dot(g,g.T)+eps)**2
        #gamma = g_2/w_pls@D@w_pls.T
        #gamma = (np.linalg.norm((x-y@w_pls))**2)/(w_pls@D@w_pls.T)
        #lA = gamma
        reg = I+lA/((y.T@y))*D
        w = scipy.linalg.solve(reg.T, w_pls.T, assume_a='sym').T  # ~10 times faster
        # Normalize w
        w /= np.sqrt(np.dot(w,w.T)+eps)
        x_score = np.dot(x,w.T)
        ...
        w_pls_diff = w - w_pls_old
        #if(np.dot(w_pls_diff,w.T) < tol):
            #break
        w_pls_old = w
...
def dipals(x, y, xs, xt, A, l,iter):
        ...
        for i in range(A):
            ...
            #compute w_pls using NIPALS
            y_mask = np.all(np.abs(y) < 10*y_eps, axis = 0)
            y[:,y_mask] = 0.0
            D = convex_relaxation(xs, xt)
            w,opt_l[i] = w_pls_nipals_di(x,y,D,lA,I,iter)
